...
TodoList::TodoList() { filename = nullptr; }

TodoList::TodoList(const char* filename) { this->filename = filename; }
...
void TodoList::display()
{
  ...
  for (int i = 0; i < list.size(); ++i)
    cout << i + 1 << ") " << setw(W) << left << list[i].text()
         << (list[i].is_done() ? "Done" : "") << endl;
}
...
int TodoList::get_count() { return list.size(); }
